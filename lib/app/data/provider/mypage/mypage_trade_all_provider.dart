  ...
  dio() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return MyPageTradeAllBaseResponseModel.fromJson(response.data);
      } else {
...
class MyPageTradeUpdateProvider {
  dio({dynamic requestModel}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        logger.d(response.data);
        return MyPageTradeBaseResponseModel.fromJson(response.data);
      } else {
        throw Exception('faild to load request');
      }
    } on DioError catch (e) {
...
class MyPageTradeDeleteProvider {
  dio({dynamic requestModel}) async {
    final Dio dio = Dio();
