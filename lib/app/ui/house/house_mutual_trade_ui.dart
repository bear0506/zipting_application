...
class HouseMutualTradeUi extends GetView<HouseMutualTradeController> {
  const HouseMutualTradeUi({Key? key}) : super(key: key);
...
class ManageMentHouseWidget extends GetView<HouseMutualTradeController> {
...
class ManageMentHouseContentWidget extends GetView<HouseMutualTradeController> {
        ...
        itemBuilder: (BuildContext _, int index) => InkWell(
          ...
          child: InkWell(
            onTap: () => globalTradeRequestDialogWidget(
              ...
              context: context,
              content:
                  "나의 ${controller.houses[index].title} 집으로\r\n교환 요청하시겠습니까?",
              button: "교환요청",
              action: () async {
                ...
                if (controller.isUpdateProvider.value) {
                  Get.toNamed(
                      "/house/trade/success?title=교환요청 완료&content=교환 요청 메세지가 전송되었습니다!\r\n메세지 함을 확인해주세요",
                      preventDuplicates: false);
                } else {
