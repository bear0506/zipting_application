...
class HouseSingleTradeUi extends GetView<HouseSingleTradeController> {
  const HouseSingleTradeUi({Key? key}) : super(key: key);
...
class ManageMentHouseWidget extends GetView<HouseSingleTradeController> {
  const ManageMentHouseWidget({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) => controller.houses.isEmpty
...
class ManageMentHouseContentWidget extends GetView<HouseSingleTradeController> {
  ...
  Widget build(BuildContext context) => ListView.builder(
        ...
        itemBuilder: (BuildContext _, int index) => InkWell(
          ...
          child: InkWell(
            onTap: () => globalTradeRequestDialogWidget(
              controller: controller,
              context: context,
              content:
                  "${controller.houses[index].title} 집으로\r\n단일 여행 요청하시겠습니까?",
              button: "단일여행 요청",
              action: () async {
                ...
                if (controller.isUpdateProvider.value) {
                  Get.toNamed(
                      "/house/trade/success?title=단일 여행 요청 완료&content=단일 여행 요청 메세지가 전송되었습니다!\r\n메세지 함을 확인해주세요",
                      preventDuplicates: false);
                } else {
